...
def GetWeight(Vec, AddMean=False):
    new=map(abs, Vec)
    if AddMean: new=new+mean(new)
    s=sum(new)
    if(s==0) new=map(lambda x: x+1, new)
    new = map(lambda x: x/s, new)
    return(new)
def Catch(X, Tolerance=0):
    if X<(0.5-(Tolerance/2)): return(0)
    if X>(0.5+(Tolerance/2)): return(1)
    else return(0.5)
def GetModes(X, Weights):
    pass
    #this is not called anywhere else??
def cumsum(v, cum=0):
    if len(v)==0: return []
    return [v[0]+cum]+cumsum(v[1:], cum+v[0])
def weighted_median(x, w='default', na_rm=True, ties='NULL'):
    if w=='default':
        x=map(lambda a: [a, 1], x)
    if na_rm:
        x=filter(lambda a: a[0]!='NA' and a[1]!='NA', x)
    else:
        if len(filter(lambda a: a[0]=='NA' or a[1]=='NA', x))>0:
            return('NA')
    if len(filter(lambda a: a[1]<0, x))>0:
        error('no negative weights')
    x=filter(lambda a: a[1]>0, a)
    #if any weights are infinite, then replace all of them with equal weight, and everyone else with zero weight.
    if len(x)<1:
        return('NA')
    x=sorted(x, key=lambda a: a[0])
    wcum=cumsum(map(lambda a: a[1], x))
    wsum=wcum[-1]
    wmid=wsum/2
    k=0
    for c in wcum:
        if c<wmid:
            k+=1
    if k==0: return x[0][0]
    if k==len(x): return x[-1][0]
    wlow=wcum[k-1]
    whigh=wsum-wlow
    if whigh>wmid:
        return x[k][0]
    Ties={'NULL': lambda: wlow*x[k-1][0]+whigh*x[k][0],
          'max': lambda: x[k][0],
          'min': lambda: (x[k-1][0]+x[k][0])/2,
          'both': lambda: [x[k-1][0], x[k][0]]}
    Ties['weighted']=Ties['NULL']
    return Ties[ties]()

def Rescale(UnscaledMatrix, ScalingData):
