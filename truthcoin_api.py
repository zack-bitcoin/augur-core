"""
This is the internal API for truthshell.
These are the words that are used to interact with truthcoin.
"""
import copy
import tools
import blockchain
...
import time
import networking
import txs_truthcoin
import target

def easy_add_transaction(tx_orig, privkey='default'):
    tx = copy.deepcopy(tx_orig)
    try:
        tx['count'] = tools.count(tools.db_get('address'))
    except:
        tx['count'] = 1
...
def create_jury(args): 
    if len(args)<1:
        return('not enough inputs')
    return easy_add_transaction({'type': 'create_jury', 'vote_id': args[0]})
...
def spend(args): 
    if len(args)<2:
        return('not enough inputs')
    return easy_add_transaction({'type': 'spend', 'amount': int(args[0]), 'to':args[1]})

def votecoin_spend(args):
    if len(args)<3:
        return('not enough inputs')
...
def vote_on_decision(args):
    if len(args)<3:
        return('not enough inputs')
    decision_id=args[1]
    answer=args[2]
    acc=tools.db_get(tools.db_get('address'))
...
def SVD_consensus(args):
    if len(args)<1:
        return('unique id for that branch?')
    vote_id=args[0]
    jury=tools.db_get(vote_id)
    k=txs_tools.decisions_keepers(vote_id, jury)
    if k=='error':
...
def blockcount(args):
    return str(tools.db_get('length'))

def txs(args):
    return str(tools.db_get('txs'))
...
def my_balance(args, address='default'): 
    if address=='default':
        address=tools.db_get('address')
    return(str(tools.db_get(address)['amount']-txs_tools.cost_0(tools.db_get('txs'), address)['truthcoin_cost']))

def balance(args): 
    if len(args)<1:
        return('what address do you want the balance for?')
    return str(my_balance(args, args[0]))
...
def commands(args):
    return str(sorted(Do.keys()+['start', 'new_address', 'make_PM', 'buy_shares']))
def mine(args):
...
Do={
    ...
    'spend':spend,
    'votecoin_spend':votecoin_spend,
    'collect_winnings':collect_winnings,
    'help':help_, 'blockcount':blockcount,
    'txs':txs,
    'balance':balance,
